...
class AdoClient {
  ...
  _buildSearchTiers(keywords, areaLevel2, excludeId, lookbackDays = 365) {
    const areaFilter = areaLevel2
      ? `AND [System.AreaPath] UNDER '${areaLevel2}'`
      : '';
    ...
    if (keywords.length >= 3) {
      const conditions = keywords.slice(0, 3).map(k => `[System.Title] CONTAINS '${k}'`).join(' AND ');
      tiers.push({ tier: 'narrow', query: `${base} AND ${conditions} ORDER BY [System.CreatedDate] DESC` });
    }

    // Medium: first 2 keywords ANDed
    if (keywords.length >= 2) {
      const conditions = keywords.slice(0, 2).map(k => `[System.Title] CONTAINS '${k}'`).join(' AND ');
    ...
    if (keywords.length >= 1) {
      ...
      const conditions = `[System.Title] CONTAINS '${specific}'`;

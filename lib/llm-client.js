...
class LlmClient {
  ...
  async generateNmiReply(ticketData, analysis) {
    ...
    reply = syncReviewFooterFlags(reply);
    return reply;
  }
 ...
 * authoritative ❗ state for each numbered question, rewrite the LLM's own
 * "⚠️ Review notes" footer bullets (e.g. "❗7 flagged: ...") so they agree
 * with that final state instead of parroting the LLM's pre-correction
 * reasoning. Without this, the footer can flatly contradict the actual
 * numbered question list — e.g. the body correctly leaves Q7 unflagged
 * while the footer still says "❗7 flagged: ... per rule ..." — confirmed
 * on ticket #3024329. Only rewrites the leading "❗N flagged"/"N flagged"/
 * "N not flagged" token of each footer bullet; the rest of the bullet's
 * explanation text is left untouched.
 */
function syncReviewFooterFlags(reply) {
  ...
  // you..." — always followed by a period right after the number). Footer
  // bullets use a different shape ("❗3 flagged: ..." — no period after the
  // number), so the two never get confused with each other.
  const flagged = {};
  ...
  return reply.replace(/(❗?)\s*(\d+)\s+(not\s+)?flagged\b/gi, (match, bang, num) => {
    const qNum = Number(num);
    if (!(qNum in flagged)) return match; // unknown question number, leave as-is
    return flagged[qNum] ? `❗${qNum} flagged` : `${qNum} not flagged`;
  });
}

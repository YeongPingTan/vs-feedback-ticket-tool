...
// Cache token to avoid calling az cli on every request
let tokenCache = { token: null, expiresAt: 0 };

function getAzToken() {
  // Return cached token if still valid (with 2-min buffer)
  if (tokenCache.token && Date.now() < tokenCache.expiresAt - 120000) {
    return tokenCache.token;
  }

  try {
    const result = execSync(
      `az account get-access-token --resource "${AZURE_DEVOPS_RESOURCE}" --query "{token:accessToken, expires:expiresOn}" -o json`,
